...
def show_calender_heatmap(df, datefield, what_to_show_, where, percentile_colomap_max=95, number_of_columns=1,log=False,):
    ...
    for what_to_show in what_to_show_:
        ...
        colomap_min=df[what_to_show].min()
        ...
        fig = month_calplot(df,
                    ...
                    y=what_to_show,
                    colorscale="purples",
                    title = f"{what_to_show} - {where}",
                    showscale=True,
                )
        st.plotly_chart(fig)
      
        c = number_of_columns  # number of columns you want
        #colomap_min= None
        for i in range(0, len(years), c):
            cols = st.columns(c)  # create c columns in the current row

            for j, year in enumerate(years[i:i+c]):
                last_year = (year == years[-1])
                df_year = df[df[datefield].dt.year == year].copy()
                # if last_year:
                #     colomap_min = colomap_min
                # else:
                #     colomap_min=df_year[what_to_show].min()
