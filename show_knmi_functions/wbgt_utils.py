...
def wind_2m(u_hm: float, h_m: float = 10.0, stability: str = "D", setting: str = "rural") -> float:
    """
    Schat windsnelheid op 2m hoogte via de machtsformule (EPA power-law).

    u_hm      : gemeten windsnelheid op hoogte h_m [m/s]
    h_m       : meethoogte [m], standaard 10m
    stability : Pasquill stabiliteitsklasse A–F (standaard D = neutraal)
    setting   : 'rural' of 'urban'

    Returns: windsnelheid op 2m [m/s]
    """
    EXPONENTS = {
        #         urban  rural
        "A":    (0.15,  0.07),
    ...
    p = EXPONENTS[stability.upper()][0 if setting == "urban" else 1]
    return u_hm * (2.0 / h_m) ** p
...
def _nat_bol_temp_simpel(temp_c: float, rh_pct: float) -> float:
...
def _globe_temp_simpel(temp_c: float, wind_ms: float, q_wm2: float) -> float:
...
def wbgt_buiten(
    ...
    if dt is None:
        dt = _dt.utcnow()
    return wbgt_liljegren_from_station_opus(
...
def wbgt_schaduw(temp_c: float, rh_pct: float) -> float:
    ...
    Returns:
        WBGT in °C.
    """
    tnw = _nat_bol_temp_simpel(temp_c, rh_pct)
...
def wbgt_bereken_df(df: pd.DataFrame, stn: int = 260) -> pd.DataFrame:
    """Bereken WBGT voor een KNMI-dagdata DataFrame. gebruikt wbgt_liljegren_from_station_opus
    ...
    # P_station = P_zeeniveau * exp(-hoogte / 8500)
    hoogte_m=2
  
    result["pressure_hpa"] = (result["P"] / 10.0)  * math.exp(-hoogte_m / 8500.0)
...
def maak_wbgt_maand_barchart(df: pd.DataFrame) -> go.Figure:
    ...
    Returns:
        Plotly Figure.
    """
    st.info("Replicatie van Figuur 6: Relatieve verdeling van de dagelijkse hittekracht per maand (De Bilt, 1991–2025)")
    df = df.copy()
...
def referentie_tabel(lat,lon,dt_ref):
    ...
    for rh in rhs:
        ...
        for t in temps:
            ...
            waarde = wbgt_liljegren_opus(float(t), float(rh), wind, q, lat,lon, dt_ref)
...
def feels_like_all(temp_c: float, rh_pct: float, wind_ms: float, q_wm2: float, lat: float, lon: float, utc_dt: datetime) -> dict:
    ...
    """
    return {
        "wbgt_buiten":  round(wbgt_liljegren_opus(temp_c, rh_pct, wind_ms, q_wm2, lat, lon, utc_dt), 1),
...
def main_():
    ...
    if save_df:
        ...
        fname_out = f"wbgt_knmi_script_begin_eind.csv"
        df_result.to_csv(fname_out)
        st.info("Info downloaded")
        st.stop()
    if only_dagmax:
