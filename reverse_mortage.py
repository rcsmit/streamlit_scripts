...
class PensionCalculator:
    ...
    def interface(self):
        ...
        self.monthly_compensation_original = st.sidebar.number_input("monthly_compensation (current pricelevel):", value=self.monthly_compensation_original)
    ...
    def calculate_pension(self, num_simulations=1000):
        ...
        """
        deceased_ages, saldo_at_death_values, results = [],[],[]
        ...
        for _ in range(num_simulations):
            ...
            annual_compensation_values = []
            balance_values = []
            interest_values = []
            value_house_values = []
            balance = 0 #self.house_value_at_start
            ...
            year_simuation = self.current_year
            ...
            for j in range(1, self.max_age - self.current_age + 1):
                ...
                if person_alive:
                    ...
                    if age>100:
                        ...
                        probability_to_die = df_prob_die[str(year_simuation)].to_numpy()[df_prob_die['age'].to_numpy() == age].item()
                    if random.random() <= probability_to_die:
                                balance += value_house
                                deceased_ages.append(age)
                                saldo_at_death_values.append(int(balance))
                                
                                person_alive = False
                                                    

                    annual_compensation_values.append(int(12 * monthtly_compensation))
                    balance_values.append(int(balance))
                    interest_values.append(int((100+ annual_return_rate) / 100))
                    value_house_values.append(value_house)
                    if not person_alive:
                        break
            ...
            results.append({
                'annual_compensation_values': annual_compensation_values,
        ...
        if sum(saldo_at_death_values) > 0:
            st.info(f"Average saldo at the death of  {num_simulations} persons ({self.sexe}) : {round(sum(saldo_at_death_values)/len(saldo_at_death_values))}\nProfit for pension funds : {round(sum(saldo_at_death_values))}")# - SD {round(np.std(saldo_at_death_values),1)}")
        else:
            st.info(f"Average saldo at the death of  {num_simulations} persons ({self.sexe}) : {round(sum(saldo_at_death_values)/len(saldo_at_death_values))}\nLoss for pension funds : {round(sum(saldo_at_death_values))}")
        col1,col2= st.columns(2)
        with col1:
            self.show_ages_at_death(num_simulations, self.sexe, self.deceased_ages,self.current_age)
        with col2:
            # with st.expander("Extra info"):
            # Create a DataFrame
            df = pd.DataFrame({
                'Deceased Ages': deceased_ages,
                'Saldo at Death': saldo_at_death_values
            })


            # Group by 'Deceased Ages' and 'Saldo at Death' to count occurrences
            summary_df = df.groupby(['Deceased Ages', 'Saldo at Death']).size().reset_index(name='Count')
    ...
    def show_ages_at_death(self, num_simulations,sexe, deceased_ages, current_age):
        ...
        all_ages = pd.DataFrame({'ages':  range(self.current_age, max(self.deceased_ages)+ 1)})
